...
(function($)
{
	...
	window.bookneticInitBookingPage = function ( booking_panel_JS_element )
	{
		...
		let booknetic = {
			...
			refreshNonce: function ( callback )
			{
				...
				if ( ! window.bookneticNonceRefreshingPromise )
				{
					window.bookneticNonceRefreshingPromise = new Promise( function ( resolve )
					{
						$.ajax( {
							url:    BookneticData.ajax_url,
							method: 'POST',
							data:   { action: 'bkntc_ping', tenant_id: BookneticData.tenant_id }
						} ).done( function ( rawResult )
						{
							let result;
							try { result = JSON.parse( rawResult ); } catch ( e ) { result = rawResult; }

							if ( result && result['status'] === 'ok' && result['nonce'] )
							{
								BookneticData.ajax_nonce = result['nonce'];
							}
						} ).always( function ()
						{
							window.bookneticNonceRefreshingPromise = null;
							resolve();
						} );
					} );
				}

				return window.bookneticNonceRefreshingPromise.then( callback );
			},

			ensureFreshNonce: function ( callback )
			{
				callback = typeof callback === 'function' ? callback : function(){};

				if ( window.bookneticNonceInitialized )
				{
					callback();
					return;
				}

				window.bookneticNonceInitialized = true;
				booknetic.refreshNonce( callback );
			},
			...
			ajax: function ( action , params , func , loading, fnOnError, async_param )
			{
				...
				if ( isFormData )
				{
					...
					params.append( '_ajax_nonce', BookneticData.ajax_nonce );
				}
				else
				{
					params = { action: 'bkntc_' + backend, tenant_id: tenantId, _ajax_nonce: BookneticData.ajax_nonce, ...params };
				}
				...
				{
					if ( isFormData )
						params.set( '_ajax_nonce', BookneticData.ajax_nonce );
					else
						params[ '_ajax_nonce' ] = BookneticData.ajax_nonce;
				};
				...
				const performRequest = () =>
				{
					...
					} ).fail( function ( jqXHR )
					{
						...
						{
							nonceRetried = true;
							booknetic.refreshNonce( function ()
							{
								applyNonce();
								performRequest();
							} );
						...
						fnOnError({ status: 'error', error_msg: jqXHR.status + ' error!', jqXHR: jqXHR });
			...
			ajaxRest: (endpoint, method = "GET", data = {}, {noLoading = false} = {}) => {
				...
				return $.ajax({
					...
					method,
					data,
					headers: {"X-Wp-Nonce": BookneticData.rest_nonce},
				})
					.always(() => {
				...
				formData: function ()
				{
					...
					booking_panel_js.find('input[name]#bkntc_input_name, input[name]#bkntc_input_surname, input[name]#bkntc_input_email, input[name]#bkntc_input_phone').each(function()
					{
                        ...
                        if(name === 'phone'){
                            ...
                            value = itiInstance.getNumber(bookneticIntlTelInput.utils?.numberFormat?.E164 ?? 0);
                        }
			...
			confirmAppointment: async function ()
			{
				...
				try {
					...
					// rows are inserted before the gateway call; release the slot they still hold
					booknetic.releaseUnpaidAppointment( null, false );
				...
				{
					let expiringWindow = booknetic.paymentWindow;

					// a retry opens a new window; only close the one this confirm belongs to
					setTimeout( function ()
					{
						if ( expiringWindow === booknetic.paymentWindow && ! expiringWindow.closed )
							expiringWindow.close();
				...
				} else if ( ! booknetic.paymentStatus ) {
					// inline-complete methods (e.g. gift card) already finished during confirm; don't poll and re-fire completion
			...
			resetPaymentState: function ()
			{
				...
				booknetic.ajaxResultConfirmStep  = null;
				booknetic.paymentId              = null;
			},

			releaseUnpaidAppointment: function ( onDone, loading )
			{
				onDone = typeof onDone === 'function' ? onDone : function () {};

				if( ! booknetic.paymentId )
				{
					onDone();
					return;
				}
				...
				booknetic.ajax(
					...
					loading !== false,
					// keep the id and leave the error screen up: those rows still hold the
					// timeslot, and on a default install nothing else ever releases them
					function () {}
				);
			},
			...
			{
				// Map our display tokens (yyyy/mm/dd) to flatpickr's (Y/m/d).
				var fpFormat = booknetic.datePickerFormat()
					.replace('yyyy', 'Y')
					.replace('mm', 'm')
					.replace('dd', 'd');

				var minDate;
				if ( el[0].getAttribute('data-apply-min') && el[0].value )
				{
					minDate = booknetic.convertDate( el[0].value, booknetic.datePickerFormat(), 'Y-m-d' );
				}
            ...
            stopPolling: () => {
                if (!booknetic.intervalId) return;

                clearInterval(booknetic.intervalId);
                booknetic.intervalId = 0;
            },
		};
		...
		}).on('click', '.booknetic_try_again_btn', function ()
		{
			...
			let restoreConfirmStep = function ()
			{
				booknetic.resetPaymentState();
				...
				booking_panel_js.find('.booknetic_try_again_btn').fadeOut(150, function ()
				{
					...
					booking_panel_js.find('.booknetic_prev_step').css('opacity', '1');
				});
			...
			booknetic.releaseUnpaidAppointment( restoreConfirmStep );
		});
		...
		booknetic.ensureFreshNonce( function ()
		{
	...
	$(document).ready( function()
	{
		...
		{
			$( 'body' ).addClass( 'rtl booknetic_rtl' );
		}
